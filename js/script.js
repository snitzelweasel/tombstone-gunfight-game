class TombstoneGame {
    ...
    selectCharacter(cardElement) {
        ...
        // Auto-advance to game after selection (with delay for visual feedback)
        setTimeout(() => {
            this.startGame();
        }, 1000);
    }
    ...
    initializeMultiplayerGame(gameData) {
        ...
        if (window.PixelTombstoneGame) {
            ...
            const otherPlayers = Object.values(gameData.players || {});
            const opponent = otherPlayers.find(p => p.playerId !== this.multiplayerManager.playerId);
            if (opponent && opponent.character) {
                this.pixelGame.opponentCharacter = opponent.character;
            }

            console.log('Multiplayer pixel game initialized');
        }
    }
